...
def docling_ppt_to_markdown(name, src, out, models_tar = None):
    srcs = [src]
    artifacts_cmd = ""
    artifacts_flag = ""
    if models_tar != None:
...
tmp_dir="$(@D)/tmp"
rm -rf "$$tmp_dir"
mkdir -p "$$tmp_dir/in" "$$tmp_dir/pptx" "$$tmp_dir/out" "$$tmp_dir/home"
...
cp "$$input" "$$local_ppt"
$(location @libreoffice//:soffice) --headless --nologo --nolockcheck --nodefault --nofirststartwizard --convert-to pptx --outdir "$$tmp_dir/pptx" "$$local_ppt"

pptx_file="$$(echo "$$tmp_dir/pptx/"*.pptx)"
device="$${{DOCLING_DEVICE:-auto}}"
$(location @docling//:docling) --from pptx --to md --device "$$device" {artifacts_flag} --output "$$tmp_dir/out" "$$pptx_file"

md_out="$$(echo "$$tmp_dir/out/"*.md)"
cp "$$md_out" "$@"
""".format(
            artifacts_cmd = artifacts_cmd,
            artifacts_flag = artifacts_flag,
